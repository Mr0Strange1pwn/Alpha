...
function JobDetails() {
  ...
  const { designations, roles, employeeInfo , employeeJobDetails } = useSelector(store => store.emp)
  ...
  const handleSave = () => {
    ...
    if(details.jobType && details.shiftTimeFrom && details.shiftTimeTo  && details.weekelyHoliday && details.probationPeriod){
      ...
      if(employeeJobDetails.user_profile_id){
        ...
        dispatch(uploadJobDetails(formData, history, setCurrentStep,backpackClick))
      }
    }
  }
  const inputStyle = {
        ...
        <div className="row">
            <div className="col-sm-12 col-md-6 col-lg-6">
              ...
              >
                ...
                {jobTypes.map(job=>  <option value={job.id}>{job.Type_name}</option>)}
            ...
            <div className="col-sm-12 col-md-6 col-lg-6">
              ...
              <Timeclock setTimevalue={time=>  SetDetails({...details, shiftTimeFrom: time})} value={details.shiftTimeFrom} />
              </div>
              <div className="col-sm-12 col-md-6 col-lg-6">
              <label class="form-check-label reg-lable" for="exampleCheck1">
                Shift Timing To
              </label>
              <Timeclock setTimevalue={time=>  SetDetails({...details, shiftTimeTo: time})} value={details.shiftTimeTo} />
              </div>
              </div>
              {/* <TimePicker/> */}
              {/* <input
