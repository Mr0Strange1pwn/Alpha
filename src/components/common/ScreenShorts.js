...
import { getEmpSS, getEmpworkTime } from "../../redux/actions/screenshortsAction";
...
export default function ScreenShorts() {
  ...
  const { empName , empID } = useParams();
  const {  workTime , empSS } = useSelector((store) => store.screenshorts);
  ...
  useEffect(() => {
    dispatch(getEmpworkTime(empID,moment(startDate).format("DD/MM/YYYY")));
  }, [startDate]);
  ...
  // const filterSS =()=>{
  //   let ss = empSS.filter(data=> moment(data.datetime_of_capture).format("DD/MM/YYYY")  === moment(startDate).format("DD/MM/YYYY") )
  //   console.log("ss ", ss , startDate );
  //   setScreenShorts(ss)
  // }
  ...
  return (
    <>
      ...
      <h5> <span className="ss-total-time">{empName}</span>  Screen Shorts</h5>
        </div>
